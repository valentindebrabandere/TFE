...
function getDockElement(): HTMLElement | null {
  ...
  return (
    dockComponent.shadowRoot
      ? dockComponent.shadowRoot.querySelector(".js-dock")
      : dockComponent
  ) as HTMLElement;
}
...
// Modify the animDockAddEvtList function to include the click event
...
function addAnim(evt: Event, iconAnimDock: Function) {
  ...
  let appAfter: Element | null =
    currentApp < arrDockAppsDyn.length - 1
      ? arrDockAppsDyn[currentApp + 1]
      : null;
  iconAnimDock(appBefore, evt.currentTarget, appAfter);
}

function iconAnimDockIn(
  appBefore: Element | null,
  currentApp: EventTarget | null,
  appAfter: Element | null
) {
